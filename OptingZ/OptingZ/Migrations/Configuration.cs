namespace OptingZ.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<OptingZ.DAL.OptingzDbContext>
    {
        ...
        protected override void Seed(OptingZ.DAL.OptingzDbContext context)
        {
            ...
            foreach (ProductStickerMaster e in productstickers)
            {
                var productSticker = context.ProductStickerMasters.Where(
                    s => s.ProductMasterID == e.ProductMasterID &&
                         s.StickerMasterID == e.StickerMasterID).SingleOrDefault();
                if (productSticker == null)
                {
                    context.ProductStickerMasters.Add(e);
                    context.SaveChanges();
                }
            }
            ...
            foreach (ProductCategoryMaster e in productcategories)
            {
                var productCategory = context.ProductCategoryMasters.Where(
                    s => s.ProductMasterID == e.ProductMasterID &&
                         s.CategoryMasterID == e.CategoryMasterID && s.SubCategoryMasterID == e.SubCategoryMasterID).SingleOrDefault();
                if (productCategory == null)
                {
                    context.ProductCategoryMasters.Add(e);
                    context.SaveChanges();
                }
            }
